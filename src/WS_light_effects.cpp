...
void WSwriteProcent (int procent, int team, int action) {
   ...
   if (team == 1 and ( action == 2 or action == 1)) {
    LedsOn = 12 - LedsOn;
    for (int i=0; i < 12; i++) {
      if (i <= LedsOn) {
        leds[i] = CRGB::Green;
        ...
        leds[i+12] = CRGB::DarkSlateGrey;
      }
    }    
  }

  else if (team == 2 and ( action == 2 or action == 1)) {
    LedsOn = 12 - LedsOn;
    for (int i=0; i < 12; i++) {
      if (i <= LedsOn) {
        leds[i] = CRGB::Red;
